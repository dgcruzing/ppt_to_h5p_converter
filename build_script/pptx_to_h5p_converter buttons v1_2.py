...
def convert_slides_to_images(ppt_file, resolution):
    ...
    try:
        ...
        for i, slide in enumerate(presentation.Slides):
            ...
            slide.Export(image_path, "PNG", resolution[0], resolution[1])
...
def convert_ppt_to_h5p(ppt_file, h5p_file, resolution):
    try:
        temp_dir, image_paths = convert_slides_to_images(ppt_file, resolution)
    except Exception as e:
...
def create_gui():
    ...
    def convert():
        ...
        resolution = (1920, 1080) if resolution_var.get() == "High" else (1280, 720)
        
        try:
            convert_ppt_to_h5p(input_file, output_file, resolution)
    ...
    tk.Label(root, text="Resolution:").grid(row=2, column=0, sticky="w", padx=5, pady=5)
    tk.Radiobutton(root, text="High (1920x1080)", variable=resolution_var, value="High").grid(row=2, column=1, sticky="w", padx=5, pady=5)
    tk.Radiobutton(root, text="Low (1280x720)", variable=resolution_var, value="Low").grid(row=3, column=1, sticky="w", padx=5, pady=5)

    tk.Button(root, text="Convert", command=convert).grid(row=4, column=1, pady=20)
